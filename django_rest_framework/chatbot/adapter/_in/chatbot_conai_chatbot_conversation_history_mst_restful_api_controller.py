...
from ...application.service.chatbot_conai_chatbot_conversation_history_mst_restful_service import ChatbotConaiChatbotConversationHistoryMstRestfulService
...
class ChatbotConaiChatbotConversationHistoryMstRestfulApiController(APIView):
    """
    # CLASS : ChatbotConaiChatbotConversationHistoryMstRestfulApiController
    # AUTHOR : conai
    # TIME : 2024. 12. 24. 오후 12:53
    # DESCRIPTION
        - ChatbotConaiChatbotConversationHistoryMstRestfulApiController
        - CONAI_CHATBOT_CONVERSATION_HISTORY_MST API
        - 디렉터리 : chatbot
        - 서비스 설명 : CONAI_CHATBOT_CONVERSATION_HISTORY_MST API
        - 서비스 호출 url : /chatbot/conaiChatbotConversationHistoryMst
        - 서비스 호출 method : GET POST PUT DELETE
        - 서비스 호출 body : json
        - 서비스 호출 파라미터 :
            conaiChatbotConversationHistoryMstId (Primary key for the conversation history table)
            conversationId (Related conversation)
            historyData (JSONB field storing the conversation history)
            createdAt (Timestamp when the history was created)
            updatedAt (Timestamp when the history was last updated)
    ...
    swaggeer_tags = ['CONAI TAROT SYSTEM - ChatbotConaiChatbotConversationHistoryMst Restful API']
    swagger_operation_summary = "ChatbotConaiChatbotConversationHistoryMst Restful GET API"
    swagger_operation_description_add = "## 추가 정보\n" \
                                        "\n" \
                                        "### 필요한 내용 추가 정의 하여 사용\n" \
                                        "\n" \
                                        "PARAMETERS INFO\n" \
                                        "Table: CONAI_CHATBOT_CONVERSATION_HISTORY_MST\n" \
                                        "| Column Name                         | Data Type | Primary Key | Description                               |\n" \
                                        "|:-----------------------------------:|:---------:|:-----------:|:-----------------------------------------:|\n" \
                                        "| conaiChatbotConversationHistoryMstId | UUID      | Yes         | Primary key for the conversation history table |\n" \
                                        "| conversationId                      | UUID      | No          | Related conversation                      |\n" \
                                        "| historyData                         | JSON      | No          | JSONB field storing the conversation history |\n" \
                                        "| createdAt                           | DATETIME  | No          | Timestamp when the history was created     |\n" \
                                        "| updatedAt                           | DATETIME  | No          | Timestamp when the history was last updated |\n"
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.service = container.get(ChatbotConaiChatbotConversationHistoryMstRestfulService)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         ...
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_HISTORY_MST Restful GET API 요청\n" + swagger_operation_description_add,
                         query_serializer=ConaiChatbotConversationHistoryMstGetSerializer(),
                         responses={
                             200: openapi.Response(
                                 ...
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiChatbotConversationHistoryMstGetSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
    ...
    def get(self, request):
        ...
        result = self.service.chatbot_conai_chatbot_conversation_history_mst_get(string_params)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         ...
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_HISTORY_MST Restful POST API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiChatbotConversationHistoryMstPostSerializer,
                         responses={
                             200: openapi.Response(
                                 description='Success',
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiChatbotConversationHistoryMstPostSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = self.service.chatbot_conai_chatbot_conversation_history_mst_post(request.data)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         operation_summary=swagger_operation_summary,
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_HISTORY_MST Restful PUT API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiChatbotConversationHistoryMstPutSerializer,
                         responses={
                             200: openapi.Response(
                                 description='Success',
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiChatbotConversationHistoryMstPutSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
                             400: openapi.Response(description='Bad Request'),
    ...
    def put(self, request, *args, **kwargs):
        ...
        result_object = self.service.chatbot_conai_chatbot_conversation_history_mst_put(request.data)
        ...
        if isinstance(serializer.initial_data, list):
            ...
            bulk_serializer = ConaiChatbotConversationHistoryMstPutSerializer(ordered_instances, data=serializer.initial_data, many=True, partial=True)
            ...
            for idx, child_serializer in enumerate(bulk_serializer.child.initial_data):
                ...
                single_serializer = ConaiChatbotConversationHistoryMstPutSerializer(instance, data=child_serializer, partial=True)
                ...
                serialized_data = ConaiChatbotConversationHistoryMstPutSerializer(updated_instance).data
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         operation_summary=swagger_operation_summary,
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_HISTORY_MST Restful DELETE API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiChatbotConversationHistoryMstDeleteSerializer,
                         responses={200: openapi.Response(
                             description='Success',
                             examples={
                                 'application/json': {
                                     'success': True,
                                     'data': ConaiChatbotConversationHistoryMstDeleteSerializer().data  # Use your serializer here
                                 }
                             }
                         ),
    ...
    def delete(self, request, *args, **kwargs):
        ...
        tag_object = self.service.chatbot_conai_chatbot_conversation_history_mst_delete(request.data)
