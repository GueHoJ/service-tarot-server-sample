...
from ...application.service.chatbot_conai_tarot_service_default_message_mst_restful_service import ChatbotConaiTarotServiceDefaultMessageMstRestfulService
...
class ChatbotConaiTarotServiceDefaultMessageMstRestfulApiController(APIView):
    """
    # CLASS : ChatbotConaiTarotServiceDefaultMessageMstRestfulApiController
    # AUTHOR : conai
    # TIME : 2025. 1. 14. 오후 12:45
    # DESCRIPTION
        - ChatbotConaiTarotServiceDefaultMessageMstRestfulApiController
        - CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST API
        - 디렉터리 : chatbot
        - 서비스 설명 : CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST API
        - 서비스 호출 url : /chatbot/conaiTarotServiceDefaultMessageMst
        - 서비스 호출 method : GET POST PUT DELETE
        - 서비스 호출 body : json
        - 서비스 호출 파라미터 :
                conaiTarotServiceDefaultMessageMstId (Primary key and unique identifier for each message)
    messageType (Type of the message (e.g., "system", "trigger"))
    messageContent (Content of the system or trigger message)
    version (Version identifier for the message)
    isActive (Indicates whether this message is currently active)
    description (Description of the message or its purpose)
    ...
    swaggeer_tags = ['CONAI TAROT SYSTEM - ChatbotConaiTarotServiceDefaultMessageMst Restful API']
    swagger_operation_summary = "ChatbotConaiTarotServiceDefaultMessageMst Restful GET API"
    swagger_operation_description_add = "## 추가 정보\n" \
                                        "\n" \
                                        "### 필요한 내용 추가 정의 하여 사용\n" \
                                        "\n" \
                                        "PARAMETERS INFO\n" \
                                        "CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST API 테이블\n" \
                                        "|NAME                           | TYPE    | PK   |DESC                                               |ETC     |\n" \
                                        "|:-----------------------------:|:-------:|:----:|:-------------------------------------------------:|:------:|\n" \
                                        "|conaiTarotServiceDefaultMessageMstId |UUID    |   V  |Primary key and unique identifier for each message|-      |\n" \
                                        "|messageType                   |CHAR     |      |Type of the message (e.g., 'system', 'trigger')   |-      |\n" \
                                        "|messageContent                |TEXT     |      |Content of the system or trigger message          |-      |\n" \
                                        "|version                       |CHAR     |      |Version identifier for the message                |-      |\n" \
                                        "|isActive                      |BOOLEAN  |      |Indicates whether this message is currently active|-      |\n" \
                                        "|description                   |TEXT     |      |Description of the message or its purpose         |-      |\n" \
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.service = container.get(ChatbotConaiTarotServiceDefaultMessageMstRestfulService)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         ...
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST Restful GET API 요청\n" + swagger_operation_description_add,
                         query_serializer=ConaiTarotServiceDefaultMessageMstGetSerializer(),
                         responses={
                             200: openapi.Response(
                                 ...
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiTarotServiceDefaultMessageMstGetSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
    ...
    def get(self, request):
        ...
        result = self.service.chatbot_conai_tarot_service_default_message_mst_get(string_params)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         ...
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST Restful POST API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiTarotServiceDefaultMessageMstPostSerializer,
                         responses={
                             200: openapi.Response(
                                 description='Success',
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiTarotServiceDefaultMessageMstPostSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = self.service.chatbot_conai_tarot_service_default_message_mst_post(request.data)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         operation_summary=swagger_operation_summary,
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST Restful PUT API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiTarotServiceDefaultMessageMstPutSerializer,
                         responses={
                             200: openapi.Response(
                                 description='Success',
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiTarotServiceDefaultMessageMstPutSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
                             400: openapi.Response(description='Bad Request'),
    ...
    def put(self, request, *args, **kwargs):
        ...
        result_object = self.service.chatbot_conai_tarot_service_default_message_mst_put(request.data)
        ...
        if isinstance(serializer.initial_data, list):
            ...
            bulk_serializer = ConaiTarotServiceDefaultMessageMstPutSerializer(ordered_instances, data=serializer.initial_data, many=True, partial=True)
            ...
            for idx, child_serializer in enumerate(bulk_serializer.child.initial_data):
                ...
                single_serializer = ConaiTarotServiceDefaultMessageMstPutSerializer(instance, data=child_serializer, partial=True)
                ...
                serialized_data = ConaiTarotServiceDefaultMessageMstPutSerializer(updated_instance).data
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         operation_summary=swagger_operation_summary,
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_TAROT_SERVICE_DEFAULT_MESSAGE_MST Restful DELETE API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiTarotServiceDefaultMessageMstDeleteSerializer,
                         responses={200: openapi.Response(
                             description='Success',
                             examples={
                                 'application/json': {
                                     'success': True,
                                     'data': ConaiTarotServiceDefaultMessageMstDeleteSerializer().data  # Use your serializer here
                                 }
                             }
                         ),
    ...
    def delete(self, request, *args, **kwargs):
        ...
        tag_object = self.service.chatbot_conai_tarot_service_default_message_mst_delete(request.data)
