...
from ...application.service.chatbot_conai_chatbot_conversation_mst_restful_service import ChatbotConaiChatbotConversationMstRestfulService
...
class ChatbotConaiChatbotConversationMstRestfulApiController(APIView):
    """
    # CLASS : ChatbotConaiChatbotConversationMstRestfulApiController
    # AUTHOR : conai
    # TIME : 2024. 12. 24. 오후 2:27
    # DESCRIPTION
        - ChatbotConaiChatbotConversationMstRestfulApiController
        - CONAI_CHATBOT_CONVERSATION_MST API
        - 디렉터리 : chatbot
        - 서비스 설명 : CONAI_CHATBOT_CONVERSATION_MST API
        - 서비스 호출 url : /chatbot/conaiChatbotConversationMst
        - 서비스 호출 method : GET POST PUT DELETE
        - 서비스 호출 body : json
        - 서비스 호출 파라미터 :
            1. **conaiChatbotConversationMstId**: Primary key for the conversation table.
            2. **sessionId**: Identifier for the conversation session.
            3. **userId**: Identifier for the user participating in the conversation.
            4. **createdAt**: Timestamp when the conversation was created.
            5. **updatedAt**: Timestamp when the conversation was last updated.
    ...
    swaggeer_tags = ['CONAI TAROT SYSTEM - ChatbotConaiChatbotConversationMst Restful API']
    swagger_operation_summary = "ChatbotConaiChatbotConversationMst Restful GET API"
    swagger_operation_description_add = "## 추가 정보\n" \
                                        "\n" \
                                        "### 필요한 내용 추가 정의 하여 사용\n" \
                                        "\n" \
                                        "PARAMETERS INFO\n" \
                                        "Table: CONAI_CHATBOT_CONVERSATION_MST\n" \
                                        "| Column Name                  | Data Type | Primary Key | Description                                |\n" \
                                        "|:----------------------------:|:---------:|:-----------:|:------------------------------------------:|\n" \
                                        "| conaiChatbotConversationMstId| UUID      | Yes         | Primary key for the conversation table     |\n" \
                                        "| sessionId                    | UUID      | No          | Identifier for the conversation session    |\n" \
                                        "| userId                       | CHAR      | No          | Identifier for the user participating      |\n" \
                                        "| createdAt                    | DATETIME  | No          | Timestamp when the conversation was created|\n" \
                                        "| updatedAt                    | DATETIME  | No          | Timestamp when the conversation was last updated|\n"
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.service = container.get(ChatbotConaiChatbotConversationMstRestfulService)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         ...
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_MST Restful GET API 요청\n" + swagger_operation_description_add,
                         query_serializer=ConaiChatbotConversationMstGetSerializer(),
                         responses={
                             200: openapi.Response(
                                 ...
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiChatbotConversationMstGetSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
    ...
    def get(self, request):
        ...
        result = self.service.chatbot_conai_chatbot_conversation_mst_get(string_params)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         ...
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_MST Restful POST API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiChatbotConversationMstPostSerializer,
                         responses={
                             200: openapi.Response(
                                 description='Success',
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiChatbotConversationMstPostSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = self.service.chatbot_conai_chatbot_conversation_mst_post(request.data)
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         operation_summary=swagger_operation_summary,
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_MST Restful PUT API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiChatbotConversationMstPutSerializer,
                         responses={
                             200: openapi.Response(
                                 description='Success',
                                 examples={
                                     'application/json': {
                                         'success': True,
                                         'data': ConaiChatbotConversationMstPutSerializer().data  # Use your serializer here
                                     }
                                 }
                             ),
                             400: openapi.Response(description='Bad Request'),
    ...
    def put(self, request, *args, **kwargs):
        ...
        result_object = self.service.chatbot_conai_chatbot_conversation_mst_put(request.data)
        ...
        if isinstance(serializer.initial_data, list):
            ...
            bulk_serializer = ConaiChatbotConversationMstPutSerializer(ordered_instances, data=serializer.initial_data, many=True, partial=True)
            ...
            for idx, child_serializer in enumerate(bulk_serializer.child.initial_data):
                ...
                single_serializer = ConaiChatbotConversationMstPutSerializer(instance, data=child_serializer, partial=True)
                ...
                serialized_data = ConaiChatbotConversationMstPutSerializer(updated_instance).data
    ...
    @swagger_auto_schema(tags=swaggeer_tags,
                         operation_summary=swagger_operation_summary,
                         operation_description="# CONAI TAROT SEVER에서 CONAI TAROT DB 로 CONAI_CHATBOT_CONVERSATION_MST Restful DELETE API 요청\n" + swagger_operation_description_add,
                         request_body=ConaiChatbotConversationMstDeleteSerializer,
                         responses={200: openapi.Response(
                             description='Success',
                             examples={
                                 'application/json': {
                                     'success': True,
                                     'data': ConaiChatbotConversationMstDeleteSerializer().data  # Use your serializer here
                                 }
                             }
                         ),
    ...
    def delete(self, request, *args, **kwargs):
        ...
        tag_object = self.service.chatbot_conai_chatbot_conversation_mst_delete(request.data)
